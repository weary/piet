...
import pietlib, shlex, simplejson, urllib.request, urllib.parse, urllib.error, re, gps
...
def maps_lookup(van, naar):
	...
	url = """http://maps.google.nl/maps/api/directions/json?""" + urllib.parse.urlencode(google_args)
	try:
...
def location(address):
	...
	url = "http://maps.google.com/maps/api/geocode/json?" + urllib.parse.urlencode(google_args)
	try:
		result = simplejson.loads(pietlib.get_url(url, maxsize=10*1024*1024))
	except Exception as e:
		fail("exception: %r" % e)
...
def Distance(params):
	...
	url="http://www.indo.com/cgi-bin/dist?" + urllib.parse.urlencode({
